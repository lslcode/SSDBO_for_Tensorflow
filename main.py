...
def testing():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if os.path.exists('./session_params/session.ckpt.index') :
            ...
            pred_class, pred_location = ssdbo_model.run(image,None)
...
def training():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        while(min_loss_class > 1 and running_count < 100000):
            running_count += 1
            
            train_data, actual_data,_ = get_traindata_voc2012(batch_size)
            if len(train_data) > 0:
                loss_all, f_class = ssdbo_model.run(train_data, actual_data)
